...
def BF(theta, gamma, lumbda, array_rf):
    ...
    g = np.array([np.exp(0.02 + 0.02 * np.random.normal()) 
                for i in range (10000)]) #Check the function of g
    
    #Utility from financial gain or loss
    def nu_R(R):
        return [i - array_rf if i >= array_rf else lumbda * (i - array_rf) for i in R]
    
    #Error term
    def e_term(x, b_0):
        return theta * b_0 * np.mean(nu_R(x*g)) + theta * x - 1 #check function of error term
        #this e_term formula assumes
    
    #Bisection search
    b_0 = np.linspace(0, 10, 101) #check the linspace value (0, 10, 101?)
    x_values = []
    ...
    for i in b_0:
        x_term(1, 1.1, i) #check the value of xneg,xpos (1,1.1?)
